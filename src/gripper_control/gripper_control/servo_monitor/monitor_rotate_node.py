from . import servo_monitor_node   # 匯入你自訂的 servo_monitor_node (父類別 ServoMonitorNode)
import rclpy                    # ROS 2 Python API
from std_msgs.msg import Float32  # 訂閱的訊息型態（角度指令會是 Float32）
import time                     # 讓你能使用 time.sleep() 做延遲

# MonitorRotateNode（繼承自 ServoMonitorNode）
class MonitorRotateNode(ServoMonitorNode):
    def __init__(self):
        super().__init__('servo_rotate_monitor')
...
def monitor_rotate_main(args=None):
    rclpy.init(args=args)
    node = MonitorRotateNode()
    try:
        rclpy.spin(node)
    finally:
        node.cleanup()
